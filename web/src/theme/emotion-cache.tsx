  ...
  useServerInsertedHTML(() => {
    ...
    inserted.forEach(({ name, isGlobal }) => {
      ...
      if (typeof style !== 'boolean') {
        if (isGlobal) globals.push({ name, style: style || '' });
        else {
          styles += style;
      ...
      <>
        {
          globals.map(({ name, style }) => (
            <style key={ name } data-emotion={ `${ registry.cache.key }-global ${ name }` }
              dangerouslySetInnerHTML={ { __html: options.prepend ? `@layer emotion {${ style }}` : style } } />
          ))
        }
        {
          styles ?
            <style data-emotion={ dataEmotionAttribute } dangerouslySetInnerHTML={ { __html: options.prepend ? `@layer emotion {${ styles }}` : styles } } />
            :
            <></>
        }
      </>
